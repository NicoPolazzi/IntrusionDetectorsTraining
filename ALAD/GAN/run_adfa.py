...
import importlib
import sys
import gan.adfa_utilities as network
import data.adfa as data
...
def create_logdir(method, weight, rd):
    ...
    return "gan/train_logs/adfa/{}/{}/{}".format(weight, method, rd)


def train_and_test(nb_epochs, weight, method, degree, random_seed):
    """ Runs the Bigan on the ADFA dataset
    ...
    """
    logger = logging.getLogger("GAN.train.adfa.{}".format(method))
    ...
    sv = tf.compat.v1.train.Supervisor(logdir=logdir, save_summaries_secs=None,
                             save_model_secs=120)
...
#            sess.graph._unsafe_unfinalize() #ADDED
        ...
        attacks=np.zeros((GENERATED_ATTACKS, 3))
        ...
        np.save('/home/polazzi/eGAN/Efficient-GAN-Anomaly-Detection/data/adfa_generated_attacks.npy', attacks)
        ...
        fill = np.ones([batch_size - size, 3])
