...
def _get_dataset():
    ...
    CICIDS_train_X_no_attack=np.load('/home/polazzi/datasets/cicids/train_X_no_attack.npy' )
    CICIDS_test_X= np.load('/home/polazzi/datasets/cicids/test_X.npy')
    CICIDS_test_Y=np.load('/home/polazzi/datasets/cicids/test_Y.npy' )
    
    #df_train
    df1=pd.DataFrame(CICIDS_train_X_no_attack)
    df1=df1.replace([np.inf, -np.inf], -1) 
    df1=df1.fillna(-1, inplace=False)
    
    df2=pd.DataFrame(np.zeros((CICIDS_train_X_no_attack.shape[0], 1)), columns=["label"])
    frame=[df1, df2]
    df_train=pd.concat(frame, axis=1)

    #df_train
    df1=pd.DataFrame(CICIDS_test_X)
    df1=df1.replace([np.inf, -np.inf], -1) 
    df1=df1.fillna(-1, inplace=False)
    df2=pd.DataFrame(CICIDS_test_Y, columns=["label"])
    frame=[df1, df2]
